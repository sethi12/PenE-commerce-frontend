    ...
    const fetchProducts = async () => {
      try {
        ...
        if (response.ok) {
          ...
          // Filter to only include products where condition is "sale"
          const saleList = productList.filter(
            (product) => product.condition && product.condition.toLowerCase() === "sale"
          );
          
          setProducts(saleList);
        }
      } catch (error) {
    ...
    <div className="min-h-screen bg-paper px-6 py-24 lg:px-10">
      <div className="mx-auto max-w-7xl">
        ...
        <div className="mb-16 flex flex-col items-start gap-4">
          <span className="label-mono">Limited offerings</span>
          ...
          <p className="max-w-xl font-body text-sm text-graphite sm:text-base">
            Exceptional writing instruments offered at privileged rates from our workshop.
          </p>
        </div>
        ...
        {products.length === 0 ? (
          <div className="py-24 text-center font-body text-sm text-graphite">
            No sale instruments currently available in the archive.
          </div>
        ) : (
